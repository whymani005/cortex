...
extension NSDate {
    ...
    var firstDayOfTheMonth: NSDate {
        var date: NSDate?
        NSCalendar.currentCalendar().rangeOfUnit(NSCalendarUnit.Month, startDate:&date , interval: nil, forDate: self)
        return date!
    }
    ...
    var monthDayAndYearComponents: NSDateComponents {
        ...
        return calendar.components([.Year, .Month, .Day], fromDate: self)
