...
class CalendarLogic: Hashable {
    ...
    private var numberOfVisibleDaysforFollowingMonth: Int {
        ...
        _ = NSCalendar.currentCalendar().dateFromComponents(parts)
    ...
    private var calculateCurrentMonthVisibleDays: [Date] {
        ...
        //for var i = 1; i <= numberOfDaysInMonth; i += 1 { //C-GONE
    ...
    private var calculatePreviousMonthVisibleDays: [Date] {
        ...
        var i = numberOfDaysInMonth - (numberOfVisibleDays - 1)
        while(i <= numberOfDaysInMonth) {
            dates.append(Date(day: i, month: parts.month, year: parts.year))
            i += 1
        }
        
        /*for var i = numberOfDaysInMonth - (numberOfVisibleDays - 1); i <= numberOfDaysInMonth; i += 1 { //C-GONE
    ...
    private var calculateFollowingMonthVisibleDays: [Date] {
        ...
        _ = numberOfVisibleDaysforFollowingMonth
        let parts  = date.monthDayAndYearComponents
        
        //for var i = 1; i <= numberOfVisibleDaysforFollowingMonth; i += 1 { //C-GONE
        for i in 1...numberOfVisibleDaysforFollowingMonth {
            dates.append(Date(day: i, month: parts.month, year: parts.year))
        }
        return dates
    }
