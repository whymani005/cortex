...
class TrendsRepository {
    ...
    func getNumThoughtsArrayForLastNDaysFromGivenDate(inputDate: NSDate, numOfDays: Int) -> [Int] {
        var countArray : [Int] = []
        
        for day in 0...numOfDays {
            let dateToGet = DateUtils.plusDays(inputDate, daysToAdd: -day)
            let dayThoughtsCount = dataRepo.getThoughtCountForDateRange(dateToGet, endDate: dateToGet)
            countArray.append(dayThoughtsCount)
        }
